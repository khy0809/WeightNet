...
def test_tencent_ml_classes():
    classes, synsets, hierarchy, subsets = datasets.tencent_ml.TencentML7M.get_classes(lang='ko', use_subset=True)
    assert len(classes) == 2457
    assert len(synsets) == 2457
    assert len(hierarchy) == 2457

    classes, synsets, hierarchy, subsets = datasets.tencent_ml.TencentML7M.get_classes(use_subset=True)
    assert len(classes) == 2457
    assert len(synsets) == 2457
    assert len(hierarchy) == 2457
...
def test_tencent_ml_validation():
    t0 = time.time()
    ds = datasets.tencent_ml.TencentML7M('validation', use_subset=True)
    ...
    image, labels = ds[idx]
    check.equal(image.size, (500, 375))
    check.equal(labels, [ds.subsets[idx] for idx in [3371, 2609, 1833, 1054, 1041, 865, 2]])

    idx = 4824 // 2 - 1
    image, labels = ds[idx]
    check.equal(image.size, (375, 500))
    check.equal(labels, [ds.subsets[idx] for idx in [2095, 2094, 2092, 2065, 1905, 1829, 1054, 1041, 865, 2]])

    idx = 4824 - 1
    image, labels = ds[idx]
    check.equal(image.size, (375, 500))
    check.equal(labels, [ds.subsets[idx] for idx in [4858, 4822, 4781, 4767, 4765, 1067, 1041, 865, 2]])

    idx = 4824
    image, labels = ds[idx]
    check.equal(image.size, (1024, 683))
    check.equal(labels, [ds.subsets[idx] for idx in [5173, 5170, 1042, 865, 2, 11026, 892, 890, 884, 870, 859, 5851, 5193, 5181, 9303, 9300, 9289, 1043, 11057, 9305]])

    idx = 4824 + 38739 // 2 - 1
    image, labels = ds[idx]
    check.equal(image.size, (1024, 686))
    check.equal(labels, [ds.subsets[idx] for idx in [6866, 6854, 6781, 6767, 6522, 6519, 6468, 5174, 5170, 1042, 865, 2, 6460, 6880, 6878]])

    idx = 4824 + 38739 -1
    image, labels = ds[idx]
    check.equal(image.size, (1024, 601))
    check.equal(labels, [ds.subsets[idx] for idx in [5173, 5170, 1042, 865, 2, 5851, 5193, 5181, 1314, 1300, 1192, 1053, 1041]])
