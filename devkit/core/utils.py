...
def save_checkpoint(model_dir, state, is_best):
    step = state['step']
    path = os.path.join(model_dir, 'model.pth-' + str(step))
...
def load_state(model_dir, model, optimizer=None):
    if not os.path.exists(model_dir + '/checkpoint'):
        ...
        if optimizer != None:
            ...
            print("=> also loaded optimizer from checkpoint '{}' (step {})"
                  .format(model_dir, start_step))
            return best_prec1, start_step


def load_state_step(model_dir, model, step):
    model_path = model_dir + '/model.pth-' + str(step)
    checkpoint = torch.load(model_path,map_location='cuda:{}'.format(torch.cuda.current_device()))
